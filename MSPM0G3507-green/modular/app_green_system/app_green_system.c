...
void GreenSystem_FormatParams(char *buffer, size_t buffer_size)
{
    ...
    (void) snprintf(buffer, buffer_size,
        "system control_ms=%lu first_ms=%lu err_px=%ld q2_offset_x_px=%ld q2_offset_y_px=%ld q2_target_err_px=%ld q2_min_sep_px=%ld q2_first_control_ms=%lu q2_follow_control_ms=%lu %s",
...
void GreenSystem_FormatStatus(char *buffer, size_t buffer_size)
{
    ...
    (void) snprintf(buffer, buffer_size,
        "state=%s phase=%s task=%s qstate=%s q2=%s q2_ms=%lu src=%s vision=%u online=%u stale=%u stale_ms=%lu frame=%lu track_ms=%lu target=%u target_xy=%ld,%ld target_frame=%lu red=%u green=%u red_xy=%ld,%ld green_xy=%ld,%ld last_red=%u last_red_xy=%ld,%ld last_red_frame=%lu last_red_ms=%lu err=%ld,%ld dist_valid=%u dist=%ld q1_target_dist_pt=%ld q2_rg_dist_pt=%ld target_ok=%u q2_in_band=%u q2_sep_ok=%u success=%u failure=%u first_ok=%u run_ms=%lu first_ms=%lu fail=%lu score_failed=%u reason=%s indicator=%s motion=%s",
    ...
    int16_t red_x, int16_t red_y, int16_t green_x, int16_t green_y)
{
    ...
    target_x = (int32_t) red_x + gGreenParams.q2_offset_x_px;
    target_y = (int32_t) red_y + gGreenParams.q2_offset_y_px;
