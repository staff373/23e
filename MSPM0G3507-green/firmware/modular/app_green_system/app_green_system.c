...
#define GREEN_SYSTEM_MOTION_STATUS_SIZE (256U)
#define GREEN_SYSTEM_INDICATOR_STATUS_SIZE (48U)
#define GREEN_SYSTEM_PARAM_STATUS_SIZE (1200U)
...
#define GREEN_SYSTEM_DEFAULT_Q2_OFFSET_X_PX (0)
#define GREEN_SYSTEM_DEFAULT_Q2_OFFSET_Y_PX (10)
#define GREEN_SYSTEM_DEFAULT_Q2_TARGET_ERR_PX (4)
#define GREEN_SYSTEM_DEFAULT_Q2_MIN_SEP_PX (8)
...
#define GREEN_SYSTEM_DEFAULT_Q2_PREDICT_MS (80U)
#define GREEN_SYSTEM_DEFAULT_Q2_PREDICT_MAX_PX (16)
#define GREEN_SYSTEM_DEFAULT_Q2_CORNER_BOOST_MS (250U)
#define GREEN_SYSTEM_DEFAULT_Q2_CORNER_DELTA_PX (4)
#define GREEN_SYSTEM_Q2_VELOCITY_MIN_DT_MS (10U)
#define GREEN_SYSTEM_Q2_VELOCITY_MAX_DT_MS (200U)
...
typedef struct {
    ...
    int32_t q2_offset_x_px;
    int32_t q2_offset_y_px;
    int32_t q2_target_err_px;
    int32_t q2_min_sep_px;
    uint32_t q2_first_control_ms;
    uint32_t q2_follow_control_ms;
    uint32_t q2_predict_ms;
    int32_t q2_predict_max_px;
    uint32_t q2_corner_boost_ms;
    int32_t q2_corner_delta_px;
} GreenSystemParams_t;
...
static uint8_t gQ2RedStepValid;
static int16_t gQ2LastRedStepX;
static int16_t gQ2LastRedStepY;
static int32_t gQ2RedVxPps;
static int32_t gQ2RedVyPps;
static uint8_t gQ2PredictionValid;
static int16_t gQ2PredRedX;
static int16_t gQ2PredRedY;
static int16_t gQ2PredDx;
static int16_t gQ2PredDy;
...
static uint8_t GreenSystem_Q2SetTarget(
    ...
    uint8_t use_prediction);
static void GreenSystem_Q2RememberRed(void);
static void GreenSystem_Q2MaybeTriggerCornerBoost(int32_t dx, int32_t dy);
static int32_t GreenSystem_Q2PredictionDelta(int32_t velocity_pps);
static void GreenSystem_Q2UpdateQuality(void);
static uint8_t GreenSystem_Q2InBandNow(void);
static uint8_t GreenSystem_Q2TargetOkNow(void);
static uint8_t GreenSystem_Q2SeparationOkNow(void);
...
static int32_t GreenSystem_ClampI32(
    int32_t value, int32_t min_value, int32_t max_value);
...
void GreenSystem_ResetParams(void)
{
    ...
    gGreenParams.q2_offset_x_px = GREEN_SYSTEM_DEFAULT_Q2_OFFSET_X_PX;
    gGreenParams.q2_offset_y_px = GREEN_SYSTEM_DEFAULT_Q2_OFFSET_Y_PX;
    gGreenParams.q2_target_err_px = GREEN_SYSTEM_DEFAULT_Q2_TARGET_ERR_PX;
    gGreenParams.q2_min_sep_px = GREEN_SYSTEM_DEFAULT_Q2_MIN_SEP_PX;
    ...
    gGreenParams.q2_predict_ms = GREEN_SYSTEM_DEFAULT_Q2_PREDICT_MS;
    gGreenParams.q2_predict_max_px = GREEN_SYSTEM_DEFAULT_Q2_PREDICT_MAX_PX;
    gGreenParams.q2_corner_boost_ms =
        GREEN_SYSTEM_DEFAULT_Q2_CORNER_BOOST_MS;
    gGreenParams.q2_corner_delta_px =
        GREEN_SYSTEM_DEFAULT_Q2_CORNER_DELTA_PX;
...
uint8_t GreenSystem_SetParam(const char *name, int32_t value)
{
    ...
    if (GreenSystem_TokenEquals(name, "q2_offset_x_px")) {
        return GreenSystem_SetSignedParamChecked(
            &gGreenParams.q2_offset_x_px, value, -240, 240);
    }
    if (GreenSystem_TokenEquals(name, "q2_offset_y_px")) {
        return GreenSystem_SetSignedParamChecked(
            &gGreenParams.q2_offset_y_px, value, -240, 240);
    }
    if (GreenSystem_TokenEquals(name, "q2_target_err_px")) {
        return GreenSystem_SetSignedParamChecked(
            &gGreenParams.q2_target_err_px, value, 1, 120);
    }
    if (GreenSystem_TokenEquals(name, "q2_min_sep_px")) {
        return GreenSystem_SetSignedParamChecked(
            &gGreenParams.q2_min_sep_px, value, 0, 240);
    }
    ...
    if (GreenSystem_TokenEquals(name, "q2_predict_ms")) {
        return GreenSystem_SetParamChecked(
            &gGreenParams.q2_predict_ms, value, 0U, 300U);
    }
    if (GreenSystem_TokenEquals(name, "q2_predict_max_px")) {
        return GreenSystem_SetSignedParamChecked(
            &gGreenParams.q2_predict_max_px, value, 0, 80);
    }
    if (GreenSystem_TokenEquals(name, "q2_corner_boost_ms")) {
        return GreenSystem_SetParamChecked(
            &gGreenParams.q2_corner_boost_ms, value, 0U, 1000U);
    }
    if (GreenSystem_TokenEquals(name, "q2_corner_delta_px")) {
        return GreenSystem_SetSignedParamChecked(
            &gGreenParams.q2_corner_delta_px, value, 0, 80);
    }
...
uint8_t GreenSystem_GetParam(const char *name, int32_t *value)
{
    ...
    } else if (GreenSystem_TokenEquals(name, "q2_offset_x_px")) {
        *value = gGreenParams.q2_offset_x_px;
    } else if (GreenSystem_TokenEquals(name, "q2_offset_y_px")) {
        *value = gGreenParams.q2_offset_y_px;
    } else if (GreenSystem_TokenEquals(name, "q2_target_err_px")) {
        *value = gGreenParams.q2_target_err_px;
    } else if (GreenSystem_TokenEquals(name, "q2_min_sep_px")) {
        *value = gGreenParams.q2_min_sep_px;
    ...
    } else if (GreenSystem_TokenEquals(name, "q2_predict_ms")) {
        *value = (int32_t) gGreenParams.q2_predict_ms;
    } else if (GreenSystem_TokenEquals(name, "q2_predict_max_px")) {
        *value = gGreenParams.q2_predict_max_px;
    } else if (GreenSystem_TokenEquals(name, "q2_corner_boost_ms")) {
        *value = (int32_t) gGreenParams.q2_corner_boost_ms;
    } else if (GreenSystem_TokenEquals(name, "q2_corner_delta_px")) {
        *value = gGreenParams.q2_corner_delta_px;
    } else {
...
void GreenSystem_FormatParams(char *buffer, size_t buffer_size)
{
    ...
    (void) snprintf(buffer, buffer_size,
        "system control_ms=%lu first_ms=%lu err_px=%ld q2_offset_x_px=%ld q2_offset_y_px=%ld q2_target_err_px=%ld q2_min_sep_px=%ld q2_first_control_ms=%lu q2_follow_control_ms=%lu q2_predict_ms=%lu q2_predict_max_px=%ld q2_corner_boost_ms=%lu q2_corner_delta_px=%ld %s",
        (unsigned long) gGreenParams.control_ms,
        (unsigned long) gGreenParams.first_lock_ms,
        (long) gGreenParams.success_err_px,
        (long) gGreenParams.q2_offset_x_px,
        (long) gGreenParams.q2_offset_y_px,
        (long) gGreenParams.q2_target_err_px,
        (long) gGreenParams.q2_min_sep_px,
        (unsigned long) gGreenParams.q2_first_control_ms,
        (unsigned long) gGreenParams.q2_follow_control_ms,
        (unsigned long) gGreenParams.q2_predict_ms,
        (long) gGreenParams.q2_predict_max_px,
        (unsigned long) gGreenParams.q2_corner_boost_ms,
        (long) gGreenParams.q2_corner_delta_px,
        motionParams);
}
...
void GreenSystem_FormatStatus(char *buffer, size_t buffer_size)
{
    ...
    success = GreenSystem_IsSuccessNow();
    target_ok = (q2_active != 0U) ? GreenSystem_Q2TargetOkNow() : success;
    q2_in_band = (q2_active != 0U) ? GreenSystem_Q2InBandNow() : 0U;
    q2_sep_ok = (q2_active != 0U) ? GreenSystem_Q2SeparationOkNow() : 0U;
    ...
    pred_valid = (q2_active != 0U) ? gQ2PredictionValid : 0U;
    pred_red_x = (q2_active != 0U) ? gQ2PredRedX : -1;
    pred_red_y = (q2_active != 0U) ? gQ2PredRedY : -1;
    pred_dx = (q2_active != 0U) ? gQ2PredDx : 0;
    pred_dy = (q2_active != 0U) ? gQ2PredDy : 0;
    red_vx = (q2_active != 0U) ? gQ2RedVxPps : 0;
    red_vy = (q2_active != 0U) ? gQ2RedVyPps : 0;

    (void) snprintf(buffer, buffer_size,
        "state=%s phase=%s task=%s qstate=%s q2=%s q2_ms=%lu src=%s vision=%u online=%u stale=%u stale_ms=%lu frame=%lu track_ms=%lu target=%u target_xy=%ld,%ld target_frame=%lu red=%u green=%u red_xy=%ld,%ld green_xy=%ld,%ld last_red=%u last_red_xy=%ld,%ld last_red_frame=%lu last_red_ms=%lu pred=%u pred_xy=%ld,%ld pred_d=%ld,%ld rv=%ld,%ld err=%ld,%ld dist_valid=%u dist=%ld q1_target_dist_pt=%ld q2_rg_dist_pt=%ld target_ok=%u q2_in_band=%u q2_sep_ok=%u success=%u failure=%u first_ok=%u run_ms=%lu first_ms=%lu fail=%lu score_failed=%u reason=%s indicator=%s motion=%s",
...
static void GreenSystem_UpdateVision(void)
{
    ...
    gVision.image_width = 0U;
    gVision.image_height = 0U;
    gVision.frame = 0U;
    gVision.track_ms = 0U;
    gVision.red_x = -1;
    gVision.red_y = -1;
    ...
    if (hasTrack) {
        ...
        gVision.image_width = track.image_width;
        gVision.image_height = track.image_height;
...
static void GreenSystem_Q2Poll(void)
{
    ...
    if (GreenSystem_Q2FreshRedGreen() != 0U) {
        ...
        has_target = GreenSystem_Q2SetTarget(gVision.red_x, gVision.red_y,
            gVision.green_x, gVision.green_y, 1U);
        ...
        has_target = GreenSystem_Q2SetTarget(gQ2LastRedX, gQ2LastRedY,
            gVision.green_x, gVision.green_y, 0U);
        if (gQ2Phase != GREEN_Q2_PHASE_RED_LOST_GRACE) {
            GreenSystem_Q2EnterPhase(
    ...
    uint8_t use_prediction)
{
    ...
    if ((use_prediction != 0U) && (gGreenParams.q2_predict_ms > 0U) &&
        (gGreenParams.q2_predict_max_px > 0) &&
        (gQ2LastRedValid != 0U)) {
        pred_dx = GreenSystem_Q2PredictionDelta(gQ2RedVxPps);
        pred_dy = GreenSystem_Q2PredictionDelta(gQ2RedVyPps);
        pred_red_x = (int32_t) red_x + pred_dx;
        pred_red_y = (int32_t) red_y + pred_dy;
        if (gGreenParams.q2_offset_y_px >= 0) {
            pred_red_y = GreenSystem_ClampI32(pred_red_y, red_y, 32767);
        } else {
            pred_red_y = GreenSystem_ClampI32(pred_red_y, 0, red_y);
        }
        gQ2PredictionValid = ((pred_dx != 0) ||
            ((pred_red_y - red_y) != 0)) ? 1U : 0U;
        gQ2PredDx = GreenSystem_ClampI16(pred_dx, -32768, 32767);
        gQ2PredDy = GreenSystem_ClampI16(pred_red_y - red_y, -32768, 32767);
        gQ2PredRedX = GreenSystem_ClampI16(pred_red_x, -32768, 32767);
        gQ2PredRedY = GreenSystem_ClampI16(pred_red_y, -32768, 32767);
    }

    target_x = pred_red_x + gGreenParams.q2_offset_x_px;
    target_y = pred_red_y + gGreenParams.q2_offset_y_px;

    gTargetLocked = 1U;
    gTargetX = GreenSystem_ClampI16(target_x, 0,
        (gVision.image_width > 0U) ? ((int32_t) gVision.image_width - 1) :
                                    32767);
    gTargetY = GreenSystem_ClampI16(target_y, 0,
        (gVision.image_height > 0U) ? ((int32_t) gVision.image_height - 1) :
                                     32767);
...
static void GreenSystem_Q2RememberRed(void)
{
    ...
    if ((gQ2LastRedValid != 0U) &&
        (gVision.frame == gQ2LastRedFrame)) {
        return;
    }

    if (gQ2LastRedValid != 0U) {
        dt_ms = (uint32_t)(gGreenMs - gQ2LastRedMs);
        dx = (int32_t) gVision.red_x - gQ2LastRedX;
        dy = (int32_t) gVision.red_y - gQ2LastRedY;

        if ((dt_ms >= GREEN_SYSTEM_Q2_VELOCITY_MIN_DT_MS) &&
            (dt_ms <= GREEN_SYSTEM_Q2_VELOCITY_MAX_DT_MS)) {
            gQ2RedVxPps = (dx * 1000L) / (int32_t) dt_ms;
            gQ2RedVyPps = (dy * 1000L) / (int32_t) dt_ms;
            GreenSystem_Q2MaybeTriggerCornerBoost(dx, dy);
            gQ2LastRedStepX = GreenSystem_ClampI16(dx, -32768, 32767);
            gQ2LastRedStepY = GreenSystem_ClampI16(dy, -32768, 32767);
            gQ2RedStepValid = 1U;
        } else {
            gQ2RedVxPps = 0;
            gQ2RedVyPps = 0;
...
static void GreenSystem_Q2MaybeTriggerCornerBoost(int32_t dx, int32_t dy)
{
    ...
    if ((gQ2RedStepValid == 0U) ||
        (gGreenParams.q2_corner_boost_ms == 0U) ||
        (gGreenParams.q2_corner_delta_px <= 0)) {
        return;
    }

    last_dx = gQ2LastRedStepX;
    last_dy = gQ2LastRedStepY;
    abs_dx = (dx >= 0) ? dx : -dx;
    abs_dy = (dy >= 0) ? dy : -dy;
    last_abs_dx = (last_dx >= 0) ? last_dx : -last_dx;
    last_abs_dy = (last_dy >= 0) ? last_dy : -last_dy;

    if ((abs_dx < gGreenParams.q2_corner_delta_px) &&
        (abs_dy < gGreenParams.q2_corner_delta_px)) {
        return;
    }

    if (((last_dx > 0) && (dx < 0)) || ((last_dx < 0) && (dx > 0)) ||
        ((last_dy > 0) && (dy < 0)) || ((last_dy < 0) && (dy > 0))) {
        corner = 1U;
    }

    if ((((last_abs_dx >= gGreenParams.q2_corner_delta_px) &&
             (abs_dy >= gGreenParams.q2_corner_delta_px) &&
             (last_abs_dy < gGreenParams.q2_corner_delta_px)) ||
            ((last_abs_dy >= gGreenParams.q2_corner_delta_px) &&
             (abs_dx >= gGreenParams.q2_corner_delta_px) &&
             (last_abs_dx < gGreenParams.q2_corner_delta_px)))) {
        corner = 1U;
    }

    if (corner != 0U) {
        GreenMotion_StartCornerBoost(gGreenParams.q2_corner_boost_ms);
    }
}
...
static int32_t GreenSystem_Q2PredictionDelta(int32_t velocity_pps)
{
    ...
    delta = (velocity_pps * (int32_t) gGreenParams.q2_predict_ms) / 1000L;
    return GreenSystem_ClampI32(delta, -gGreenParams.q2_predict_max_px,
        gGreenParams.q2_predict_max_px);
}
...
static uint8_t GreenSystem_Q2InBandNow(void)
{
    if ((GreenSystem_Q2TargetOkNow() == 0U) ||
        (GreenSystem_Q2SeparationOkNow() == 0U)) {
        return 0U;
    }
...
static uint8_t GreenSystem_Q2TargetOkNow(void)
{
    ...
    if ((gVision.stale != 0U) || (gTargetLocked == 0U) ||
        (gVision.green_valid == 0U) || (gVision.green_x < 0) ||
        (gVision.green_y < 0)) {
        return 0U;
    }

    abs_x = (gVision.err_x >= 0) ? gVision.err_x : -gVision.err_x;
    abs_y = (gVision.err_y >= 0) ? gVision.err_y : -gVision.err_y;

    return ((abs_x <= gGreenParams.q2_target_err_px) &&
        (abs_y <= gGreenParams.q2_target_err_px)) ? 1U : 0U;
}

static uint8_t GreenSystem_Q2SeparationOkNow(void)
{
    if ((gVision.stale != 0U) || (gVision.green_valid == 0U) ||
        (gQ2RedGreenDistPt < 0)) {
        return 0U;
    }

    return (gQ2RedGreenDistPt >= gGreenParams.q2_min_sep_px) ? 1U : 0U;
}
