...
#ifndef STEPPER_RESET_DIR_AFTER_STOP
#define STEPPER_RESET_DIR_AFTER_STOP (1)
#endif

#ifndef STEPPER_DIR_RESET_DELAY_MS
#define STEPPER_DIR_RESET_DELAY_MS (2U)
#endif
...
typedef struct {
    ...
    volatile uint8_t dir_reset_pending;
    volatile uint16_t dir_reset_delay_ms;
...
static void Stepper_CancelPendingDirReset(uint8_t axis);
static void Stepper_ScheduleDirReset(uint8_t axis);
...
void BSP_Stepper_Init(void)
{
    ...
    for (axis = 0U; axis < STEPPER_AXIS_COUNT; axis++) {
        ...
        gStepperAxis[axis].dir_reset_pending = 0U;
        gStepperAxis[axis].dir_reset_delay_ms = 0U;
...
void BSP_Stepper_Tick1ms(void)
{
    ...
    for (axis = 0U; axis < STEPPER_AXIS_COUNT; axis++) {
        if ((gStepperAxis[axis].dir_reset_pending != 0U) &&
            (gStepperAxis[axis].dir_reset_delay_ms > 0U)) {
            gStepperAxis[axis].dir_reset_delay_ms--;
        }
    }
}

void BSP_Stepper_Poll(void)
{
    uint8_t axis;
    ...
    for (axis = 0U; axis < STEPPER_AXIS_COUNT; axis++) {
        if ((gStepperAxis[axis].hw_ready != 0U) &&
            ...
            gStepperAxis[axis].direction_sign = 1;
            Stepper_ApplyDirection(axis);
            Stepper_CancelPendingDirReset(axis);
        }
    }
}
...
uint8_t BSP_Stepper_IsBusy(uint8_t axis)
{
    ...
    return (uint8_t) ((gStepperAxis[axis].busy != 0U) ||
        (gStepperAxis[axis].dir_reset_pending != 0U));
}
...
static void Stepper_ScheduleDirReset(uint8_t axis)
{
#if (STEPPER_RESET_DIR_AFTER_STOP != 0)
    if (gStepperAxis[axis].direction_sign < 0) {
        gStepperAxis[axis].dir_reset_pending = 1U;
        gStepperAxis[axis].dir_reset_delay_ms = STEPPER_DIR_RESET_DELAY_MS;
    } else {
        Stepper_CancelPendingDirReset(axis);
    }
#else
